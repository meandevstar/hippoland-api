...
class ProductListCreateAPIViewTestCase(APITestCase):
    ...
    def test_user_products(self):
        ...
        """
        Product.objects.create(user=self.user, name="Clean the car!")
...
class ProductDetailAPIViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.password = "you_know_nothing"
        self.user = User.objects.create_user(self.username, self.email, self.password)
        self.todo = Product.objects.create(user=self.user, name="Call Mom!")
        self.url = reverse("products:detail", kwargs={"pk": self.todo.pk})
        self.token = Token.objects.create(user=self.user)
        self.api_authentication()
    ...
    def test_todo_object_bundle(self):
        ...
        todo_serializer_data = ProductSerializer(instance=self.todo).data
    ...
    def test_todo_object_update(self):
        ...
        response_data = json.loads(response.content)
        todo = Product.objects.get(id=self.todo.id)
